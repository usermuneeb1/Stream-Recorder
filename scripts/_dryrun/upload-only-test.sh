#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  UPLOAD-ONLY TEST — user request:                                                      ║
# ║   • record a (public) stream, but for TESTING only                          ║
# ║   • upload ONLY to Gofile + Pixeldrain                                        ║
# ║   • DO NOT upload to Archive.org   (ARCHIVE_SKIP=true)                       ║
# ║   • DO NOT write to the dashboard  (update-stats / update-links SKIPPED)     ║
# ║                                                                                  ║
# ║  Uses mocked tool binaries + simulated GitHub Actions env. Proves the       ║
# ║  UPLOAD ROUTING (correct services targeted, archive + dashboard skipped).   ║
# ║  NOTE: a REAL byte-upload to gofile/pixeldrain must run on the GitHub        ║
# ║  runner (this sandbox cannot reach those hosts). The same ARCHIVE_SKIP /    ║
# ║  MEGA_SKIP env + skipping the dashboard steps is what runs for real there.  ║
# ╚══════════════════════════════════════════════════════════════════════════╝
set -uo pipefail
...
# ── user-requested upload routing ─────────────────────────────────────────────
export MEGA_SKIP="true"        # no MEGA
export ARCHIVE_SKIP="true"     # NO Archive.org upload
# (dashboard steps skipped below — update-stats / update-links not run)
...
( cd "$REPO"; source scripts/utils.sh; source scripts/discord-notify.sh; notify_live_detected ) && echo "✅ notify-live-detected -> exit 0" || { echo "❌ notify-live-detected"; overall=1; }
...
# ── dashboard steps INTENTIONALLY SKIPPED per request ──────────────────────────
echo ""
echo "════════ SKIPPING dashboard steps (update-stats / update-links) per request ════════"
( cd "$REPO"; source scripts/utils.sh; source scripts/discord-notify.sh; notify_recording_complete ) && echo "✅ discord-notify -> exit 0" || { echo "❌ discord-notify"; overall=1; }

echo ""; echo "════════ SUMMARY ════════"
echo "TMP dir: $TMP"
grep -E "GOFILE_LINKS|PIXELDRAIN_LINKS|ARCHIVE_LINKS|MEGA_LINKS|RECORDING_SUCCESS|UPLOAD_SUCCESS_COUNT" "$GITHUB_ENV" 2>/dev/null
echo "--- config: ARCHIVE_SKIP=$ARCHIVE_SKIP  MEGA_SKIP=$MEGA_SKIP  (dashboard steps SKIPPED) ---"
echo ""
if [[ $overall -eq 0 ]]; then
  echo "🎉 UPLOAD-ONLY TEST RESULT: gofile + pixeldrain only; Archive.org skipped; dashboard NOT touched"
else
  echo "⚠️  some step non-zero"
fi
