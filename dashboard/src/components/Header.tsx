    ...
    <header className="sticky top-0 z-50 backdrop-blur-2xl border-b" style={{ background: 'var(--glass)', borderColor: 'var(--bd)' }}>
      <div className="max-w-[1400px] mx-auto flex items-center gap-4 px-4 sm:px-6 lg:px-10 h-[72px]">
        ...
        <a href="#" onClick={() => { window.location.hash = ''; }} className="shrink-0">
          <img src="/logo.png" alt="Muslim Lantern Archive" className="h-10 sm:h-11 object-contain" />
        </a>
        ...
        <div className="w-full max-w-xs relative">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search streams..."
            className="w-full rounded-xl py-2.5 pl-9 pr-4 text-[13px] font-medium focus:outline-none focus:ring-2 transition-all"
            style={{ background: 'var(--bg2)', border: '1px solid var(--bd2)', color: 'var(--tx)', '--tw-ring-color': 'var(--red)' } as any} />
        </div>
        {/* Theme */}
        <button onClick={toggle} className="p-2.5 rounded-xl transition-all hover:scale-105 active:scale-95" style={{ background: 'var(--bg2)', color: 'var(--tx3)' }}>
          {theme === 'dark'
