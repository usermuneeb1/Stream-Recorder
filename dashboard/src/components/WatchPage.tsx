...
export function WatchPage({rec,onClose,all,onNav,theme,onTheme}:P){
  ...
  return(
    <div className="min-h-screen" style={{background:'var(--bg)',color:'var(--tx)'}}>
      <nav className="flex items-center justify-between px-4 sm:px-6 h-16 border-b" style={{borderColor:'var(--bd)'}}>
      ...
      <div className="flex flex-col xl:flex-row max-w-[1800px] mx-auto">
        <div className="flex-1 min-w-0">
          ...
          <div className="px-4 sm:px-5 xl:px-4 pt-4 pb-8 space-y-4">
            <div><h1 className="text-lg font-bold leading-snug">{rec.title}</h1><p className="text-xs mt-1.5 font-medium" style={{color:'var(--tx3)'}}>{rec.date} · {rec.durationFmt} · {rec.sizeHuman}{rec.resolution?.includes('1080')?' · 1080p HD':''}</p></div>
            {src.length>1&&(<div className="flex items-center gap-2 flex-wrap py-3 px-4 rounded-xl" style={{background:'var(--bg2)'}}><span className="text-[9px] font-bold uppercase tracking-[.2em] mr-1" style={{color:'var(--tx3)'}}>Source:</span>{src.map((s,i)=>(<button key={i} onClick={()=>setSi(i)} className="text-[11px] px-3 py-1.5 rounded-lg font-bold transition-all" style={{background:i===si?'var(--red)':'var(--bg4)',color:i===si?'#fff':'var(--tx2)'}}>{s.l}</button>))}</div>)}
            {dl.length>0&&(<div className="py-3 px-4 rounded-xl" style={{background:'var(--bg2)'}}><p className="text-[9px] font-bold uppercase tracking-[.2em] mb-3" style={{color:'var(--tx3)'}}>Download</p><div className="flex flex-wrap gap-2">{dl.map((d,i)=>(<a key={i} href={d.u} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-[12px] font-bold text-white hover:brightness-110 active:scale-95 transition-all" style={{background:d.c}}><svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>{d.l}</a>))}</div></div>)}
            {ch.length>0&&(<div className="py-3 px-4 rounded-xl" style={{background:'var(--bg2)'}}><p className="text-[9px] font-bold uppercase tracking-[.2em] mb-3" style={{color:'var(--tx3)'}}>Chapters · {ch.length}</p><div className="space-y-px">{ch.map((c,i)=>{const a=i===ci,g=c.label.toLowerCase().includes('joins');return(<button key={i} onClick={()=>seek(c.time)} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-all group" style={{background:a?'var(--bg4)':'transparent'}}><span className="font-mono text-[11px] w-14 shrink-0 tabular-nums font-bold" style={{color:a?'var(--red)':'var(--tx3)'}}>{ft(c.time)}</span>{g&&<span className="text-xs">👤</span>}<span className="text-[12px] flex-1 font-medium" style={{color:a?'var(--tx)':'var(--tx2)'}}>{c.label}</span>{a&&<span className="w-1.5 h-1.5 rounded-full pulse-dot shrink-0" style={{background:'var(--red)'}}/>}{!a&&<svg className="w-3 h-3 opacity-0 group-hover:opacity-30 shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>}</button>);})}</div></div>)}
            {hasChat&&chat.length>0&&(<div className="py-3 px-4 rounded-xl" style={{background:'var(--bg2)'}}><p className="text-[9px] font-bold uppercase tracking-[.2em] mb-3" style={{color:'var(--tx3)'}}>💬 Chat · {chat.length}</p><div ref={chatR} className="max-h-60 overflow-y-auto space-y-0.5 pr-1 scroll-smooth">{chat.map((m:any,i:number)=>{const mt=m.time_in_seconds||m.timestamp||0;const vis=mt<=t+2;const who=m.author?.name||m.author||'Viewer';const txt=m.message||m.text||m.body||'';if(!txt)return null;return(<div key={i} data-t={mt} className="flex gap-1.5 text-[10px] py-0.5 transition-opacity" style={{opacity:vis?1:.08}}><button onClick={()=>seek(mt)} className="font-mono w-10 shrink-0 tabular-nums hover:underline" style={{color:'var(--tx3)'}}>{ft(mt)}</button><span className="font-bold" style={{color:'var(--red)'}}>{who}</span><span style={{color:'var(--tx2)'}}>{txt}</span></div>);})}</div></div>)}
          </div>
        </div>
        {others.length>0&&(<div className="xl:w-[350px] shrink-0 border-l px-4 pb-8 pt-4" style={{borderColor:'var(--bd)'}}><p className="text-[9px] font-bold uppercase tracking-[.2em] mb-4" style={{color:'var(--tx3)'}}>Up Next</p><div className="space-y-2.5">{others.map(o=>(<button key={o.videoId} onClick={()=>{onNav(o);window.scrollTo(0,0);}} className="flex gap-2.5 w-full text-left rounded-lg p-1.5 -mx-1.5 transition-colors hover:bg-[var(--bg2)] group"><div className="w-36 shrink-0 aspect-video rounded-lg overflow-hidden" style={{background:'var(--bg3)'}}><img src={o.thumbnail} alt="" loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform" onError={(e:any)=>{e.target.src='/thumbnail.jpg';}}/></div><div className="min-w-0 pt-0.5"><p className="text-[11px] font-semibold line-clamp-2 leading-snug">{o.title}</p><p className="text-[9px] mt-1" style={{color:'var(--tx3)'}}>{o.date} · {o.durationFmt}</p></div></button>))}</div></div>)}
      </div>
    </div>
  );
}
