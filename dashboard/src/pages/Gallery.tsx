              ...
              <motion.div variants={itemVariants} key={stream.videoId}>
                <TiltCard className="group flex flex-col gap-3">
                  <Link
                    ...
                    state={{ stream }}
                    className="relative aspect-video rounded-2xl overflow-hidden bg-dark-900 shadow-lg border border-dark-200 dark:border-dark-800 hover:shadow-2xl hover:shadow-brand-500/10 transition-all duration-500"
                  >
                    <img
                    ...
                    <div className="absolute top-3 left-3 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-300">
                      {Object.entries(stream.sources).slice(0, 4).map(([key, src]) => (
                        <div
                          key={key}
                          className={`h-2 rounded-full transition-all duration-500 group-hover:px-2 ${sourceColors[key] || 'bg-gray-500'}`}
                          style={{ width: '8px' }}
                          title={src.label}
                        >
                          <span className="text-[8px] text-white font-bold opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap" />
                        </div>
                      ))}
                    </div>
